...
const MultipleSelector = ({
  ...
  // Fixed-position coordinates for the portaled menu
  ...
  const handleClickOutside = (event: MouseEvent | TouchEvent) => {
    ...
    if (!clickedInsideInput && !clickedInsideTrigger && !clickedInsideMenu) {
      setOpen(false);
  ...
  const commandFilter = React.useCallback(() => {
    ...
    return undefined; // use cmdk default
  ...
  return (
    ...
    >
      <div
        ref={triggerRef}
        className={cn(
          "border-muted-foreground py-1 focus-within:border-ring focus-within:ring-ring/50 has-aria-invalid:ring-destructive/20 dark:has-aria-invalid:ring-destructive/40 has-aria-invalid:border-destructive relative min-h-[38px] rounded-md border text-sm transition-[color,box-shadow] outline-none focus-within:ring-[3px] has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50",
          {
      ...
      >
        <div className="flex flex-wrap gap-1">
          ...
          <button
            type="button"
            onClick={() => {
              const keep = selected.filter((s) => s.fixed);
              setSelected(keep);
              onChange?.(keep);
            }}
            className={cn(
      ...
      {open &&
        ...
        createPortal(
          <div
            ref={menuRef}
            className={cn(
              " z-[1000] border rounded-md shadow-lg bg-popover text-popover-foreground",
